...
<script>
    ...
    function created () {
        ...
        dispatch('updateWidth', self.updateWidthHandler);
        dispatch('compact', self.compactHandler);
        dispatch('setDraggable', self.setDraggableHandler);
        dispatch('setResizable', self.setResizableHandler);
        dispatch('setBounded', self.setBoundedHandler);
        dispatch('setTransformScale', self.setTransformScaleHandler)
        dispatch('setRowHeight', self.setRowHeightHandler);
        dispatch('setMaxRows', self.setMaxRowsHandler);
        dispatch('directionchange', self.directionchangeHandler);
        dispatch('setColNum', self.setColNum)
    ...
    onDestroy(() => {
        ...
        dispatch('setColNum', self.setColNum);
        if (data.interactObj) {
            data.interactObj.unset() // destroy interact intance
        }
    });

    onMount(() => {
        if (data.layout.responsive && data.layout.lastBreakpoint) {
            data.cols = getColsFromBreakpoint(data.layout.lastBreakpoint, data.layout.cols);
        } else {
            data.cols = data.layout.colNum;
        }
        data.rowHeight = data.layout.rowHeight;
        data.containerWidth = data.layout.width !== null ? data.layout.width : 100;
        data.margin = data.layout.margin !== undefined ? data.layout.margin : [10, 10];
        data.maxRows = data.layout.maxRows;

        if (data.isDraggable === null) {
            data.draggable = data.layout.isDraggable;
        } else {
            data.draggable = data.isDraggable;
        }
        if (data.isResizable === null) {
            data.resizable = data.layout.isResizable;
        } else {
            data.resizable = data.isResizable;
        }
        if (data.isBounded === null) {
            data.bounded = data.layout.isBounded;
        } else {
            data.bounded = data.isBounded;
        }
        data.transformScale = data.layout.transformScale
        data.useCssTransforms = data.layout.useCssTransforms;
        data.useStyleCursor = data.layout.useStyleCursor;
        data.createStyle();
    });
    ...
    // function "$parent.margin" (margin) {
    //     if (!margin || (margin[0] == data.margin[0] && margin[1] == data.margin[1])) {
    //         return;
    //     }
    //     data.margin = margin.map(m => Number(m));
    //     createStyle();
    //     emitContainerResized();
    // }
...
//                    Add rtl support
    ...
    function autoSize() {
        ...
        data.previousW = data.innerW;
        data.previousH = data.innerH;
